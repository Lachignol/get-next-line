...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   get_next_magic_boxe[fd].c                          :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: ascordil <marvin@42.fr>                    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/11/22 14:46:37 by ascordil          #+#    #+#             */
/*   Updated: 2024/11/28 12:09:49 by ascordil         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*return_next_ligne(char **magic_boxe)
{
	...
	while (((*magic_boxe)[i] != '\n') && ((*magic_boxe)[i] != '\0'))
		i++;
	to_return = ft_substr(*magic_boxe, 0,
			i + ((*magic_boxe)[i] == '\n'));
	if (!to_return)
		return (free(*magic_boxe), *magic_boxe = NULL, NULL);
	if (((*magic_boxe)[i] == '\n') && ((*magic_boxe)[i + 1] != '\0'))
	{
		temp = ft_substr(*magic_boxe, i + 1,
				ft_strlen(*magic_boxe) - i);
		if (!temp)
...
{
	static char		*magic_boxe[MAX_FD];
	char			*buffer;
	char			*line;

	line = NULL;
	if (!magic_boxe[fd])
		magic_boxe[fd] = ft_strdup("");
	if (!magic_boxe[fd])
		return (free(magic_boxe[fd]), magic_boxe[fd] = NULL, NULL);
	if (fd == -1 || BUFFER_SIZE <= 0 || read(fd, NULL, 0) < 0)
		return (free(magic_boxe[fd]), magic_boxe[fd] = NULL, NULL);
	buffer = malloc(sizeof(char) * (BUFFER_SIZE + 1));
	if (!buffer)
		return (free(magic_boxe[fd]), magic_boxe[fd] = NULL, NULL);
	get_buffer(fd, &buffer, &magic_boxe[fd]);
	if (!magic_boxe[fd] || !*magic_boxe[fd])
		return (free(buffer), buffer = NULL
			, free(magic_boxe[fd]), magic_boxe[fd] = NULL, NULL);
	line = return_next_ligne(&magic_boxe[fd]);
